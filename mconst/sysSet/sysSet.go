...
*/
const (
	PERCENTAGE_TO_TOTAL_WIN             = 1   // 100% 历史总赢乘的百分比字段（100%那个值
	PLAYER_LOSE_RATE_AFTER_SURPLUS_POOL = 0.7 // 70%  盈余池后的玩家输百分比（70%那个值）
	COEFFICIENT_TO_TOTAL_PLAYER         = 0   // 0    玩家总数所剩的系数（0那个值)
	FINAL_PERCENTAGE                    = 0.5 // 50%  最后百分比（50%那个值）
)
